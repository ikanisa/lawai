export const AUTONOMOUS_JUSTICE_SUITE = {
  autonomous_legal_agent_suite: {
    ...
    vision:
      'A living, interactive, personal legal assistant that behaves like a seasoned avocat/juriste/greffier/procureur—guiding users through every legal & court process with actionable steps, trusted sources, and agentic tools.',
    default_language: 'fr',
...
export type AutonomousUserType =
  AutonomousJusticeSuiteManifest['users']['user_types'][number]['code'];

export function getAutonomousSuiteManifest(): AutonomousJusticeSuiteManifest {
  return AUTONOMOUS_JUSTICE_SUITE;
}

export function getAgentDefinition<C extends AutonomousAgentCode>(
  code: C,
): AutonomousJusticeSuiteManifest['agents'][C] {
...
export function listAgentCodes(): AutonomousAgentCode[] {
  return Object.keys(AUTONOMOUS_JUSTICE_SUITE.agents) as AutonomousAgentCode[];
}
...
export function getUserType(code: AutonomousUserType) {
  const entry = AUTONOMOUS_JUSTICE_SUITE.users.user_types.find(
    (type) => type.code === code,
  );
  if (!entry) {
