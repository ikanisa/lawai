...
export function validateDirectorPlanBudget(plan, logger) {
    let totalTokens = 0;
    for (const step of plan.steps ?? []) {
        const tokens = step.envelope?.budget?.tokens;
        if (typeof tokens === 'number') {
            if (tokens > MAX_STEP_TOOL_BUDGET_TOKENS) {
                logger?.warn?.({ stepId: step.id, tokens, limit: MAX_STEP_TOOL_BUDGET_TOKENS }, 'director_plan_budget_exceeded');
    ...
    if (totalTokens > MAX_PLAN_TOOL_BUDGET_TOKENS) {
        logger?.warn?.({ totalTokens, limit: MAX_PLAN_TOOL_BUDGET_TOKENS }, 'director_plan_budget_total_exceeded');
