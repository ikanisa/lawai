...
const FUNDAMENTAL_RIGHTS_KEYWORDS = [
    /discrimination/i,
    /harc[èe]lement/i,
...
export function evaluateCompliance(input) {
    ...
    if (FUNDAMENTAL_RIGHTS_KEYWORDS.some((pattern) => pattern.test(normalizedQuestion)) &&
        input.payload.risk.level === 'LOW') {
        cepejViolations.push('fundamental_rights_screening');
    }
    if ((input.payload.risk.level === 'HIGH' || friaReasons.length > 0) &&
