...
import { Agent, OpenAIProvider, defineOutputGuardrail, fileSearchTool, run as runAgent, setDefaultModelProvider, setDefaultOpenAIKey, setOpenAIAPI, tool, webSearchTool, } from '@openai/agents';
import { IRACSchema, buildWebSearchAllowlist, OFFICIAL_DOMAIN_ALLOWLIST, } from '@avocat-ai/shared';
...
import { CASE_TRUST_WEIGHTS, evaluateCaseQuality } from './case-quality.js';
import { isJurisdictionAllowed } from './access-control.js';
...
const CASE_AXES = ['PW', 'ST', 'SA', 'PI', 'JF', 'LB', 'RC', 'CQ'];
...
function normaliseJurisdiction(code) {
...
function serialiseRoutingHints(routing) {
    const summary = {
...
function createRunKey(input, routing, confidentialMode, options = {}) {
...
const RESIDENCY_ZONE_NOTICES = {
    eu: 'Résidence de données : toutes les opérations restent dans l’espace UE/EEE conformément à la politique de résidence.',
    ohada: 'Résidence de données : traitement limité aux infrastructures approuvées dans l’espace OHADA. Vérifiez les exigences locales additionnelles avant export.',
    ...
    maghreb: 'Résidence de données : stockage dans l’enveloppe Maghreb ; appliquer les restrictions locales sur les transferts transfrontaliers.',
};
const RESIDENCY_ZONE_LOCATIONS = {
    eu: 'European Union',
...
function mapResidencyZoneToLocation(zone) {
...
function buildRunNotices(payload, options) {
    ...
    if (ohadaActive) {
        notices.push({
            type: 'ohada',
            message: "Zone OHADA : appliquez en priorité les Actes uniformes et la jurisprudence CCJA avant toute règle nationale complémentaire.",
        });
    }
    if (options.confidentialMode) {
        notices.push({
            type: 'confidential',
            message: 'Mode confidentiel actif : la recherche web est désactivée et le cache hors ligne doit être purgé avant toute exportation.',
        });
    }
    return notices;
}
...
async function planRun(input, accessContext, useStub, toolLogs) {
    const planTrace = [];
    const initialRouting = (await recordPlanStep(planTrace, TOOL_NAMES.routeJurisdiction, 'Analyse de juridiction', 'Détection des juridictions pertinentes et des avertissements.', async () => detectJurisdiction(input.question, input.context))) ?? { primary: null, candidates: [], warnings: [] };
    toolLogs.push({
        name: 'preflightRouting',
        args: { question: input.question, context: input.context },
        output: initialRouting,
    });
    const ohadaInsight = await recordPlanStep(planTrace, 'ohada_preemption', 'Vérification OHADA', 'Identifie les sujets OHADA et préemption potentielle.', async () => formatOhadaInsight(input.question, initialRouting), {
        optional: true,
    ...
    const deadlineInsight = await recordPlanStep(planTrace, 'deadline_estimate', 'Estimation des délais', 'Produit une estimation initiale des délais procéduraux.', async () => formatDeadlineInsight(input.question), {
        optional: true,
        detail: (value) => (value ? { summary: value } : null),
    });
    if (deadlineInsight) {
    ...
    const synonymMap = (await recordPlanStep(planTrace, 'synonym_feedback', 'Synonymes appris', 'Injecte les synonymes validés par la boucle de learning dans la planification.', async () => fetchSynonymExpansions(initialRouting.primary?.country ?? null), {
        optional: true,
    ...
    const policyVersion = (await recordPlanStep(planTrace, 'policy_version', 'Politique active', 'Identifie la dernière version de politique/guardrail activée pour contextualiser la réponse.', async () => fetchActivePolicyVersion(), {
        optional: true,
        detail: (value) => value
            ? {
                name: value.name,
    ...
    const effectiveWebSearchMode = enforcedConfidentialMode
        ? 'disabled'
    ...
    if (useStub) {
        ...
        hybridSnippets =
            (await recordPlanStep(planTrace, 'hybrid_retrieval', 'Récupération hybride', 'Combine le vector store local et File Search pour pré-contextualiser la réponse.', async () => fetchHybridSnippets(input.orgId, input.question, initialRouting.primary?.country ?? null, synonymMap), { detail: (snippets) => summariseHybridSnippets(snippets) })) ?? [];
    ...
    const franceAnalyticsGuard = (await recordPlanStep(planTrace, 'france_judge_analytics_guard', 'Contrôle analytics magistrats', 'Vérifie si la requête enfreint l’interdiction française de profilage des juges.', async () => accessContext?.policies.franceJudgeAnalyticsBlocked === false
...
async function executeAgentPlan(agent, planner, input, hybridSnippets, telemetry) {
    ...
    if (context.confidentialMode) {
        prefaceSegments.push('Mode confidentiel activé : utilisez uniquement les documents déposés et les corpus internes ; ne lancez pas de recherche web.');
    }
    const insights = [];
    ...
    const synonymPreface = formatSynonymPreface(new Map(Object.entries(context.synonymExpansions ?? {})));
    if (synonymPreface) {
    ...
    if (context.policyVersion) {
        ...
        prefaceSegments.push(`Politique active (${context.policyVersion.activatedAt}): ${context.policyVersion.name}${note}`);
    }
    if (hybridSnippets.length > 0) {
...
const citationsAllowlistGuardrail = defineOutputGuardrail({
...
const JURISDICTION_BINDING_RULES = {
    MA: {
        jurisdiction: 'MA',
        bindingLang: 'ar',
        translationNotice: 'La version française du Bulletin officiel est une traduction informative; seule la version arabe fait foi.',
        requiresBanner: true,
    ...
    TN: {
        jurisdiction: 'TN',
        bindingLang: 'ar',
        translationNotice: 'La version arabe du Journal Officiel de la République Tunisienne fait foi; la version française est informative.',
        requiresBanner: true,
    ...
    DZ: {
        jurisdiction: 'DZ',
        bindingLang: 'ar',
        translationNotice: 'La version arabe du Journal officiel algérien est contraignante; la version française accompagne pour information.',
        requiresBanner: true,
    ...
    'CA-QC': {
        jurisdiction: 'CA-QC',
        bindingLang: 'fr/en',
        translationNotice: 'Au Québec comme au Canada, les versions française et anglaise ont la même valeur juridique. Vérifiez les deux versions lorsque cela est pertinent.',
    ...
    CA: {
        jurisdiction: 'CA',
        bindingLang: 'fr/en',
        translationNotice: 'Les textes fédéraux canadiens sont publiés dans les deux langues officielles et ont une valeur juridique équivalente.',
        requiresBanner: false,
    ...
    CH: {
        ...
        translationNotice: 'Les textes suisses doivent être vérifiés dans les versions française et allemande lorsqu’elles existent.',
        requiresBanner: false,
    ...
    RW: {
        jurisdiction: 'RW',
        bindingLang: 'rw/en',
        translationNotice: 'La Gazette officielle du Rwanda est publiée en kinyarwanda et en anglais; les versions françaises doivent être vérifiées.',
        requiresBanner: false,
        source: 'Ministry of Justice / Rwanda Law Reform Commission gazettes.',
    },
};
const DOMAIN_BINDING_RULES = {
...
async function fetchHybridSnippets(orgId, question, jurisdiction, synonyms) {
    try {
        ...
        const localSnippets = normalized
            .map((entry) => {
        ...
        if (fileIds.length > 0) {
            const { data: documentRows, error: docError } = await supabase
                .from('documents')
                .select('id, source_id, openai_file_id, name, storage_path, sources(id, title, source_url, publisher, trust_tier, eli, ecli, binding_lang, residency_zone, akoma_ntoso)')
                .eq('org_id', orgId)
...
export async function getHybridRetrievalContext(orgId, question, jurisdiction) {
...
function verifyAgentPayload(payload, options) {
    if (options.allowlistViolations.length > 0) {
        ...
        throw new Error(`Citations hors périmètre autorisé (${unique.join(', ')}). Relancer avec site:<domaine_officiel> ou escalader HITL.`);
    }
    const notes = [];
    ...
    if (payload.rules && payload.rules.length > 0) {
        ...
        if (bindingCount === 0) {
            escalate({
                ...
                message: 'Les références fournies ne sont pas marquées comme contraignantes : confirmer la source officielle avant diffusion.',
                severity: 'warning',
            });
        }
    }
    ...
    if (notice.length > 0) {
        ...
        if (!noteInjected && payload.citations.length > 0) {
            payload.citations = [
                {
                    ...payload.citations[0],
                    note: payload.citations[0].note && payload.citations[0].note.length > 0
...
function buildStubPayload(question, initialRouting, hybridSnippets) {
    ...
    if (normalized.includes('maroc') || normalized.includes('non-concurrence')) {
        return {
            ...
            application: 'La clause est licite si elle protège un intérêt légitime, reste limitée dans le temps et l’espace et prévoit une contrepartie financière proportionnée. La version arabe du Bulletin officiel fait foi; la traduction française doit être rapprochée du texte contraignant.' +
                contextSection,
            conclusion: 'La clause de non-concurrence est valable sous réserve d’un intérêt légitime démontré, d’une limitation raisonnable et d’une indemnisation adéquate, avec revue HITL pour confirmer la version arabe.',
            citations: [
                {
    ...
    if (normalized.includes('ohada') || normalized.includes('gage') || normalized.includes('sûret')) {
        return {
            ...
            application: 'Le gage doit être constaté par écrit, publié au RCCM et prévoir une description précise des stocks. La CCJA rappelle que la publicité est déterminante pour opposabilité aux tiers.' +
                contextSection,
            conclusion: 'Le gage sans dépossession est valable dès lors que le créancier procède à la publicité RCCM et maintient une description dynamique des stocks, avec suivi HITL si une particularité locale est identifiée.',
            citations: [
                {
    ...
    return {
        ...
        application: 'La responsabilité délictuelle suppose une faute du salarié, un dommage et un lien de causalité direct. L’employeur peut engager l’action contre le salarié fautif ou rechercher la responsabilité de l’employeur sur le terrain du commettant.' +
            contextSection,
        conclusion: 'Les critères de l’article 1240 du Code civil sont réunis lorsque la faute personnelle du salarié est caractérisée; une revue HITL est recommandée si une faute pénale est alléguée.',
        citations: [
            {
...
async function computeCaseQuality(orgId, jurisdiction, citations) {
    ...
    const { data: sourceRows, error: sourceError } = await supabase
        .from('sources')
        .select('id, source_url, source_type, jurisdiction_code, trust_tier, binding_lang, effective_date, created_at, political_risk_flag, court_rank, court_identifier')
        .eq('org_id', orgId)
        .in('source_url', urls);
    if (sourceError) {
    ...
    const alignmentsBySource = new Map();
    for (const entry of alignmentResult.data ?? []) {
        if (!entry?.case_source_id)
            continue;
    ...
    for (const entry of overrideResult.data ?? []) {
        if (!entry?.source_id || typeof entry.new_score !== 'number')
            continue;
    ...
    const persistedHardBlockBySource = new Map();
    const persistedScoreBySource = new Map();
    ...
    const riskOverlaysByJurisdiction = new Map();
...
function buildTrustPanel(payload, retrievalSnippets, caseQuality, verification) {
    ...
    const translationWarnings = new Set();
    const bindingNotes = {};
    ...
    const residencyCounter = new Map();
    const bindingCounter = new Map();
    let akomaArticles = 0;
...
async function loadCaseQualitySummaries(orgId, citations) {
    ...
    for (const row of scoreRows ?? []) {
        ...
        const notes = Array.isArray(notesValue)
            ? notesValue.filter((item) => typeof item === 'string')
    ...
    try {
        ...
        const retrievalSnippets = (retrievalQuery.data ?? []).map((row) => {
            ...
            return {
                ...
                trustTier: trustTier === 'T1' || trustTier === 'T2' || trustTier === 'T3' || trustTier === 'T4'
                    ? trustTier
                    : undefined,
                ...
                akomaArticleCount: typeof metadata.akomaArticleCount === 'number' && Number.isFinite(metadata.akomaArticleCount)
    ...
    if (runError || !runData) {
        if (runError?.code === '23505' && runKey) {
            const existing = await supabase
                ...
                .maybeSingle();
            if (existing.data?.id) {
                const restoredTrust = (await fetchTrustPanelForRun(existing.data.id, input.orgId, payload, verification)) ??
                    buildTrustPanel(payload, retrievalSnippets, { summaries: [], forceHitl: payload.risk.hitl_required }, verification);
...
function buildInstructions(routing, confidentialMode = false, allowedJurisdictions = []) {
    ...
    if (confidentialMode) {
        segments.push('Mode confidentiel actif : n’utilise que File Search et les documents internes ; n’ouvre pas la recherche web.');
    }
    if (allowedJurisdictions.length > 0) {
        segments.push(`Juridictions autorisées pour ce locataire: ${allowedJurisdictions
            .map((code) => code.toUpperCase())
            .join(', ')}. Refuse ou escalade au HITL si la demande sort de ce périmètre.`);
    }
    if (routing.primary) {
        segments.push(`Juridiction pressentie: ${routing.primary.country} (EU=${routing.primary.eu ? 'oui' : 'non'}, OHADA=${routing.primary.ohada ? 'oui' : 'non'}). Confiance ${Math.round(routing.primary.confidence * 100)}%. ${routing.primary.rationale}`);
    }
    if (routing.candidates.length > 1) {
        segments.push(`Autres juridictions possibles: ${routing.candidates
            .slice(1)
...
function enforceFranceJudgeAnalyticsBan(question, routing) {
    ...
    const franceContext = routing.primary?.country === 'FR' ||
    ...
    return {
        triggered: true,
        rationale: "L'article L10 du Code de l'organisation judiciaire interdit la mise à disposition de données permettant d'évaluer, d'analyser ou de prédire le comportement d'un magistrat.",
    };
}
...
function recordWebSearchTruncationTelemetry(context, telemetry, items) {
...
function normaliseHostedToolCall(item) {
    ...
    const record = raw;
    if (record.type?.toLowerCase() !== 'hosted_tool_call') {
        return null;
    }
    ...
    return { name, providerData: providerData };
}
function summariseWebSearchProviderData(providerData) {
    const stats = {
        total: 0,
        allowed: 0,
        filtered: 0,
...
function collectWebSearchStats(value, path, stats) {
...
function applyNumericStat(value, path, stats) {
...
function applyArrayStat(values, path, stats) {
    if (values.length === 0) {
        return;
    }
    if (FILTERED_KEY_PATTERN.test(path)) {
...
function buildAgent(toolLogs, telemetry, context) {
    ...
    const ohadaUniformActTool = tool({
        ...
        execute: async (input, runContext) => {
            ...
            try {
                ...
                const output = mapping && (lastJurisdiction?.ohada ?? false)
    ...
    const bindingLanguageTool = tool({
        ...
        description: 'Identifie la langue juridiquement contraignante pour une source donnée et précise si une bannière d’avertissement est requise.',
        parameters: z
            .object({
    ...
    const redlineTool = tool({
        ...
        description: 'Compare deux versions de clause ou de contrat et retourne un diff structuré avec recommandations.',
        parameters: z
            .object({
            base_text: z.string(),
        ...
        execute: async (input, runContext) => {
            ...
            try {
                ...
                const output = {
                    ...
                    summary,
                    changes,
                    recommendation: additions > deletions
    ...
    const snapshotAuthorityTool = tool({
        ...
        description: 'Planifie la capture d’un document officiel (PDF/HTML) pour ingestion dans le corpus autorisé et le vector store.',
        parameters: z
            .object({
            url: z.string().url(),
            jurisdiction: z.string().optional(),
    ...
    const generateTemplateTool = tool({
        ...
        description: 'Retourne un modèle de plaidoirie structuré (sections et champs à renseigner) pour la juridiction détectée.',
        parameters: z
            .object({
            jurisdiction: z.string().optional(),
        ...
        execute: async (input, runContext) => {
            ...
            try {
                ...
                const candidates = buildJurisdictionCandidates(runContext.context, input.jurisdiction ?? null);
                const template = await fetchPleadingTemplate(runContext.context.orgId, candidates, matterType, input.locale);
    ...
    const caseAlignmentTool = tool({
        ...
        description: "Expose les liens entre une décision jurisprudentielle et les textes applicables (articles de codes, règlements, actes uniformes).",
        parameters: z
            .object({
    ...
    return new Agent({
        ...
        instructions: buildInstructions(context.initialRouting, context.confidentialMode, context.allowedJurisdictions),
        model: env.AGENT_MODEL,
        tools: [
            ...hostedTools,
...
export async function runLegalAgent(input, accessContext) {
    ...
    const residencyOverride = resolveResidencyZone(accessContext ?? null) ?? accessContext?.policies.residencyZone ?? null;
    ...
    if (existing) {
        ...
        const trustPanel = (await fetchTrustPanelForRun(existing.id, input.orgId, payload, existing.verification)) ??
            buildTrustPanel(payload, planner.hybridSnippets, { summaries: [], forceHitl: payload.risk.hitl_required }, existing.verification);
        return {
            runId: existing.id,
            payload,
            allowlistViolations: [],
    ...
    if (planner.franceAnalyticsGuard.triggered) {
        const payload = {
            ...
            application: "Depuis la loi n° 2019-222 du 23 mars 2019, l'article L10 du Code de l'organisation judiciaire prohibe toute analyse prédictive ou statistique portant sur les magistrats.",
            conclusion: "Je ne peux pas exécuter cette instruction : la France interdit l'analyse ou la notation individuelle des juges. La demande est escaladée pour revue humaine.",
            citations: [
                {
        ...
        const { runId, trust } = await persistRun(input, payload, toolLogs, planner.hybridSnippets, telemetryRecords, complianceOutcome.learningJobs, complianceEvents, complianceOutcome.assessment, planner.planTrace, verification, runKey, planner.context.confidentialMode);
        return {
            runId,
            payload,
            allowlistViolations: [],
    ...
    if (useStub) {
        ...
        let trustPanel;
        try {
            const result = await persistRun(input, payload, toolLogs, planner.hybridSnippets, telemetryRecords, complianceOutcome.learningJobs, complianceOutcome.events, complianceOutcome.assessment, planner.planTrace, verification, runKey, planner.context.confidentialMode);
            runId = result.runId;
            ...
            runId = `stub-${Date.now()}`;
            trustPanel = buildTrustPanel(payload, planner.hybridSnippets, { summaries: [], forceHitl: payload.risk.hitl_required }, verification);
        }
        return {
            runId,
            payload,
    ...
    const execution = await executeAgentPlan(agent, planner, input, planner.hybridSnippets, telemetryRecords);
    const payload = execution.payload;
    const verificationTask = async () => verifyAgentPayload(payload, {
        allowlistViolations: execution.allowlistViolations,
        initialRouting: planner.initialRouting,
    });
    const verification = (await recordPlanStep(planner.planTrace, 'verification', 'Vérification post-exécution', 'Contrôle post-exécution des citations et de la structure IRAC.', verificationTask, {
        detail: (result) => ({
    ...
    const { runId, trust: trustPanel } = await persistRun(input, payload, toolLogs, planner.hybridSnippets, telemetryRecords, complianceOutcome.learningJobs, complianceOutcome.events, complianceOutcome.assessment, planner.planTrace, verification, runKey, planner.context.confidentialMode);
    return {
        runId,
        payload,
        allowlistViolations: execution.allowlistViolations,
...
export function getAgentPlatformDefinition() {
    ...
    return {
        name: 'avocat-francophone',
        description: 'Agent juridique francophone senior orienté IRAC, couvrant les juridictions OHADA, UE et Canada avec outils de conformité.',
