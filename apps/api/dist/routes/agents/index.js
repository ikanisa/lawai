...
import { AgentRunSchema, AgentRunRequestSchema, AgentStreamRequestSchema, ResearchStreamPayloadSchema } from '@avocat-ai/shared';
import { createResearchStream } from '../research/data.js';
const MAX_RUNS_STORED = 50;
const runStore = new Map();
const runOrder = [];
function persistRun(runId, payload) {
...
export async function registerAgentsRoutes(app, _ctx) {
    ...
    app.post('/agents/stream', async (request, reply) => {
        ...
        reply.hijack();
        const events = [...stored.events, { type: 'done', data: {} }];
