import { ResearchDeskContextSchema } from '@avocat-ai/shared';
const researchPlan = {
    id: 'plan-ohada-default',
    title: 'Analyse IRAC — Inexécution contractuelle (OHADA)',
    jurisdiction: 'OHADA',
    riskLevel: 'MED',
    riskSummary: "Risque moyen : nécessité d'assurer la preuve de l'obligation et de l'inexécution; vigilance accrue sur la compétence du juge OHADA.",
    steps: [
        {
...
export const researchDeskContext = ResearchDeskContextSchema.parse({
    plan: researchPlan,
    filters: researchFilters,
    defaultCitations: researchCitations,
    suggestions: researchSuggestions
});
export function createResearchStream(input, toolsEnabled, webSearchMode = 'allowlist') {
    const stream = [];
    ...
    const summaryIntro = "Analyse factuelle : les éléments reçus évoquent une inexécution contractuelle. L'objectif est de documenter la faute et les remèdes disponibles.";
    ...
    const riskSummary = "Sensibilisation : la compétence peut basculer vers un juge national, prévoir une revue HITL pour sécuriser la stratégie.";
    stream.push({
        type: 'token',
        data: {
            token: `${summaryIntro}\n\n`
    ...
    stream.push({
        type: 'token',
        data: {
            token: "Remèdes : engager la procédure de mise en demeure, prévoir les mesures conservatoires et préparer un dossier HITL pour validation finale."
        }
    });
    stream.push({
        type: 'risk',
...
export function cloneResearchContext() {
    return JSON.parse(JSON.stringify(researchDeskContext));
}
export function getResearchPlan() {
    return JSON.parse(JSON.stringify(researchPlan));
}
export function getResearchCitations() {
    return JSON.parse(JSON.stringify(researchCitations));
}
