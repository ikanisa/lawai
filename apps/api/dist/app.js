...
export async function createApp(options = {}) {
    ...
    const { supabase: supabaseOverride, overrides, registerWorkspaceRoutes: _registerWorkspaceRoutes = true, includeWorkspaceDomainRoutes: _includeWorkspaceDomainRoutes = true, } = options;
    ...
    const rateLimiterFactory = createRateLimiterFactory({
        ...
        supabase: rateLimitConfig.driver === 'supabase'
    ...
    if (_includeWorkspaceDomainRoutes) {
        const workspaceLimiter = context.limiters.workspace ?? rateLimiterFactory.create('workspace', rateLimitConfig.buckets.workspace);
        if (!context.limiters.workspace) {
            context.limiters.workspace = workspaceLimiter;
        }
        const workspaceGuard = context.rateLimits.workspace ??
