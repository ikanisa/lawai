import { AnalyticsClient, ErpPayablesClient, GrcClient, RegulatoryClient, TaxGatewayClient, } from './connectors/index.js';
import { workerRegistry } from './worker.js';
function connectorsMissing(payload, required) {
...
function connectorHitl(payload, missing, reason) {
    return {
        status: 'needs_hitl',
...
function buildFollowUp(base, intent, objective, inputs) {
    return {
        intent,
        domain: base.domain,
        objective,
...
async function loadConnectorMap(supabase, orgId) {
...
function resolveConnectorConfig(record) {
    if (!record?.config) {
        return null;
    }
    const raw = record.config;
    const endpoint = valueToString(raw.endpoint) ?? valueToString(raw.url);
    if (!endpoint) {
        return null;
    }
    return {
...
async function requireMaybeSingle(promise, errorCode) {
...
async function executeTax({ envelope, supabase, logger }) {
    ...
    const jurisdiction = payload.inputs?.jurisdiction ?? 'Inconnue';
    const period = payload.inputs?.period ?? 'période courante';
    try {
        switch (payload.intent) {
            case 'tax.prepare_filing': {
                const amount = payload.inputs?.amount !== undefined ? Number(payload.inputs.amount) : null;
                const currency = payload.inputs?.currency ?? 'EUR';
                ...
                await requireMaybeSingle((supabase
                ...
                }, { onConflict: 'org_id,jurisdiction,period' })
                    .select()
                    .maybeSingle()), 'tax_filing_upsert_failed');
                const followUp = buildFollowUp(payload, 'tax.check_deadline', `Confirmer la date limite pour ${jurisdiction} (${period})`, { jurisdiction, period });
                return {
                    status: 'completed',
                    output: {
            ...
            case 'tax.respond_audit': {
                await taxClient.sendAuditResponse({
                    jurisdiction,
                    period,
                    response: payload.inputs?.response ?? '',
                    evidenceIds: Array.isArray(payload.inputs?.evidenceIds)
                        ? payload.inputs?.evidenceIds
                        : undefined,
                });
                await requireMaybeSingle((supabase
                    .from('finance_tax_filings')
                    ...
                    .eq('period', period)
                    .select()
                    .maybeSingle()), 'tax_audit_update_failed');
            ...
            case 'tax.check_deadline': {
                const record = await requireMaybeSingle(supabase
                    .from('finance_tax_filings')
                    .select('due_date, status')
                    .eq('org_id', orgId)
                    .eq('jurisdiction', jurisdiction)
                    .eq('period', period)
                    .maybeSingle(), 'tax_filing_lookup_failed');
                ...
                if (!dueDate) {
                    ...
                    await requireMaybeSingle((supabase
                        ...
                        .select()
                        .maybeSingle()), 'tax_deadline_upsert_failed');
                }
                return {
                    status: 'completed',
                    output: {
...
async function executeAccountsPayable({ envelope, supabase, logger }) {
    const payload = envelope.command.payload;
    ...
    try {
        switch (payload.intent) {
            case 'ap.process_invoice': {
                ...
                const currency = payload.inputs?.currency ?? 'EUR';
                const vendor = payload.inputs?.vendor ?? 'Fournisseur';
                const invoice = await erpClient.createInvoice({
                    vendor,
                    invoiceNumber: payload.inputs?.invoiceNumber ?? null,
                    amount,
                    currency,
                ...
                await requireMaybeSingle((supabase
                    ...
                    .select()
                    .maybeSingle()), 'ap_invoice_insert_failed');
                const notices = [];
                if (amount > 10000) {
            ...
            case 'ap.schedule_payment': {
                const invoiceId = payload.inputs?.invoiceId ?? null;
                ...
                const schedule = await erpClient.schedulePayment({
                    ...
                    reference: payload.inputs?.reference ?? null,
                });
                await requireMaybeSingle((supabase
                    .from('finance_ap_invoices')
                    .update({
                    ...
                    .select()
                    .maybeSingle()), 'ap_schedule_update_failed');
                return {
                    status: 'completed',
                    output: {
...
async function executeRisk({ envelope, supabase, logger }) {
    const payload = envelope.command.payload;
    ...
    try {
        switch (payload.intent) {
            case 'risk.update_register': {
                const riskId = payload.inputs?.riskId ?? 'RISK-NEW';
                const severity = payload.inputs?.severity ?? 'medium';
                const jurisdiction = payload.inputs?.jurisdiction ?? 'UNK';
                await grcClient.upsertRisk({
                    jurisdiction,
                    severity,
                    note: payload.inputs?.note ?? undefined,
                    metadata: payload.inputs ?? {},
                });
                await requireMaybeSingle((supabase
                    .from('risk_register')
            ...
            case 'risk.assess_control': {
                const controlId = payload.inputs?.controlId ?? 'CTRL-1';
                const result = payload.inputs?.testResult ?? 'not_tested';
                ...
                await requireMaybeSingle((supabase
                    ...
                    .select()
                    .maybeSingle()), 'risk_control_insert_failed');
                if (result === 'failed') {
...
async function executeAudit({ envelope, supabase, logger }) {
    const payload = envelope.command.payload;
    const missing = connectorsMissing(payload, ['grc_platform']);
    if (missing.length > 0) {
        return connectorHitl(payload, missing);
    ...
    try {
        switch (payload.intent) {
            case 'audit.prepare_walkthrough': {
                const processName = payload.inputs?.process ?? 'Processus';
                const response = await grcClient.createWalkthrough({
                    processName,
                    summary: payload.inputs?.summary ?? undefined,
                    metadata: payload.inputs ?? {},
                });
                await requireMaybeSingle((supabase
                    ...
                    .select()
                    .maybeSingle()), 'audit_walkthrough_insert_failed');
                return {
                    status: 'completed',
                    output: {
            ...
            case 'audit.update_pbc': {
                const item = payload.inputs?.item ?? 'PBC Item';
                ...
                await requireMaybeSingle((supabase
                    ...
                    .select()
                    .maybeSingle()), 'audit_pbc_insert_failed');
                return {
                    status: 'completed',
                    output: {
...
async function executeCfo({ envelope, supabase, logger }) {
    const payload = envelope.command.payload;
    ...
    try {
        switch (payload.intent) {
            case 'cfo.generate_board_pack': {
                const period = payload.inputs?.period ?? 'Dernier mois';
                const metricsInput = payload.inputs?.metrics ?? {};
                ...
                await requireMaybeSingle((supabase
                    ...
                    summary: payload.inputs?.summary ?? null,
                }, { onConflict: 'org_id,period' })
                    .select()
                    .maybeSingle()), 'board_pack_upsert_failed');
                return {
                    status: 'completed',
                    output: {
            ...
            case 'cfo.run_scenario': {
                const scenario = payload.inputs?.scenario ?? 'baseline';
                const assumptions = payload.inputs?.assumptions ?? {};
                const response = await analyticsClient.runScenario({
                    scenario,
                    assumptions,
                    period: payload.inputs?.period ?? undefined,
                });
                await requireMaybeSingle((supabase
                    .from('finance_board_packs')
                    .upsert({
                    org_id: orgId,
                    period: payload.inputs?.period ?? 'courant',
                    status: 'draft',
                    metrics: response.outputs ?? assumptions,
                    summary: `Scénario ${scenario}`,
                }, { onConflict: 'org_id,period' })
                    .select()
                    .maybeSingle()), 'board_scenario_upsert_failed');
                return {
                    status: 'completed',
                    output: {
...
async function executeRegulatory({ envelope, supabase, logger }) {
    const payload = envelope.command.payload;
    ...
    const jurisdiction = payload.inputs?.jurisdiction ?? 'Inconnue';
    const filingType = payload.inputs?.filing ?? 'Déclaration';
    ...
    try {
        switch (payload.intent) {
            case 'regulatory.prepare_filing': {
                ...
                }, { onConflict: 'org_id,jurisdiction,filing_type' })
                    .select()
                    .maybeSingle()), 'regulatory_filing_upsert_failed');
                return {
                    status: 'completed',
                    output: {
            ...
            case 'regulatory.track_deadline': {
                const record = await requireMaybeSingle(supabase
                    ...
                    .maybeSingle(), 'regulatory_filing_lookup_failed');
                ...
                if (!dueDateValue) {
                    ...
                    await requireMaybeSingle((supabase
                        ...
                        .select()
                        .maybeSingle()), 'regulatory_filing_status_upsert_failed');
                }
                return {
                    status: 'completed',
                    output: {
...
function ensureConnectorActive(payload, map, name, expectedType) {
...
export function registerFinanceWorkers(registry = workerRegistry) {
