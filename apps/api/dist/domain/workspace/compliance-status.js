...
import { fetchAcknowledgementEvents, mergeDisclosuresWithAcknowledgements, summariseAcknowledgements, toStringArray, } from './compliance';
...
export function registerComplianceStatusRoute(app, ctx, onRequest) {
    const { supabase } = ctx;
    app.get('/compliance/status', { schema: complianceStatusSchema, onRequest }, async (request, reply) => {
