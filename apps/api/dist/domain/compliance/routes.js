...
async function fetchAcknowledgementEvents(ctx, orgId, userId) {
    const { supabase } = ctx;
...
async function recordAcknowledgementEvents(ctx, request, orgId, userId, records) {
    if (records.length === 0) {
        return;
    }
    const { supabase } = ctx;
...
function summariseAcknowledgements(access, events) {
...
function mergeDisclosuresWithAcknowledgements(assessment, acknowledgements) {
...
export async function registerComplianceRoutes(app, ctx, options = {}) {
    ...
    app.get('/compliance/acknowledgements', { preHandler: acknowledgementsLimiter ? [acknowledgementsLimiter] : undefined }, async (request, reply) => {
    ...
    app.post('/compliance/acknowledgements', { preHandler: acknowledgementsLimiter ? [acknowledgementsLimiter] : undefined }, async (request, reply) => {
        const userHeader = request.headers['x-user-id'];
        if (!userHeader || typeof userHeader !== 'string') {
            return reply.code(401).send({ error: 'unauthorized' });
        }
    ...
    app.get('/compliance/status', { preHandler: statusLimiter ? [statusLimiter] : undefined }, async (request, reply) => {
        const userHeader = request.headers['x-user-id'];
        if (!userHeader || typeof userHeader !== 'string') {
            return reply.code(401).send({ error: 'unauthorized' });
        }
        ...
        const rawLimit = (request.query?.limit ?? '5');
