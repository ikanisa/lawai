...
// @ts-nocheck
import { z } from 'zod';
import { AgentRunSchema, AgentRunRequestSchema, AgentStreamRequestSchema, UploadResponseSchema, VoiceRunRequestSchema, VoiceSessionTokenSchema, } from '@avocat-ai/shared/pwa';
import { researchDeskContext, researchAnswerChunks, researchToolSummaries, citationsData, mattersData, hitlQueueData, corpusDashboardData, policyConfiguration, voiceConsoleContext, buildVoiceRunResponse, webSearchModes, } from './avocat-pwa-data.js';
...
export function registerAvocatPwaRoutes(app) {
    ...
    app.post('/api/agents/stream', {
        ...
        const tokenEvents = researchAnswerChunks.map((chunk) => ({
            type: 'token',
            data: { token: `${chunk} ` },
        }));
        const citationEvents = researchDeskContext.defaultCitations
            .slice(0, 2)
            .map((citation) => ({
            type: 'citation',
            data: { citation },
    ...
    app.post('/api/upload', { schema: { response: { 200: { type: 'object', additionalProperties: true } } } }, async () => {
    ...
    app.post('/api/realtime/session', { schema: { response: { 200: { type: 'object', additionalProperties: true } } } }, async () => {
