...
try {
    ...
    void Promise.resolve(dyn('./finance-workers.js')).catch(() => undefined);
}
catch { }
import { buildTransparencyReport, buildRetrievalMetricsResponse, buildEvaluationMetricsResponse, summariseCepej, summariseEvaluations, summariseHitl, summariseIngestion, summariseRuns, summariseSlo, mapLearningReports, } from './reports.js';
import { listSsoConnections, upsertSsoConnection, deleteSsoConnection, listScimTokens, createScimToken, deleteScimToken, listIpAllowlist, upsertIpAllowlist, deleteIpAllowlist, } from './sso.js';
...
import { SupabaseRateLimiter, createRateLimitPreHandler, createRateLimitGuard, enforceRateLimit, } from './rate-limit.js';
import { withRequestSpan } from './observability/spans.js';
import { listRegulatorDigestsForOrg } from './launch.js';
...
const workspaceLimiter = context.limiters.workspace ?? limiterFactory.create('workspace', rateLimitConfig.buckets.workspace);
if (!context.limiters.workspace) {
    context.limiters.workspace = workspaceLimiter;
}
const workspaceRateLimitGuard = context.rateLimits.workspace ?? createRateLimitGuard(workspaceLimiter, { name: 'workspace' });
context.rateLimits.workspace = workspaceRateLimitGuard;
const complianceLimiter = context.limiters.compliance ?? limiterFactory.create('compliance', rateLimitConfig.buckets.compliance);
if (!context.limiters.compliance) {
    context.limiters.compliance = complianceLimiter;
}
const complianceRateLimitGuard = context.rateLimits.compliance ?? createRateLimitGuard(complianceLimiter, { name: 'compliance' });
...
function normaliseFairnessJurisdiction(entry) {
    ...
    const totalRuns = toNumber(payload.totalRuns) ??
        toNumber(payload.total) ??
        toNumber(payload.total_runs) ??
        0;
    const hitlEscalations = toNumber(payload.hitlEscalations) ??
        toNumber(payload.hitl) ??
        toNumber(payload.hitl_escalations) ??
        0;
    const hitlRate = clampRate(toNumber(payload.hitlRate) ?? toNumber(payload.hitl_rate) ??
        (totalRuns > 0 ? hitlEscalations / totalRuns : null));
    const highRiskShare = clampRate(toNumber(payload.highRiskShare) ?? toNumber(payload.high_risk_share) ?? null);
    return {
        code,
        totalRuns,
        hitlEscalations,
...
function normaliseFairnessBenchmark(entry) {
    if (!entry || typeof entry !== 'object') {
        return null;
    }
    const payload = entry;
...
function normaliseFairnessOverall(value) {
    ...
    const highRiskShare = clampRate(toNumber(value.highRiskShare) ?? toNumber(value.high_risk_share) ?? null);
    const benchmarkRate = clampRate(toNumber(value.benchmarkRate) ?? toNumber(value.benchmark_rate) ?? null);
    return {
        totalRuns,
        hitlRate,
        highRiskShare,
...
function deriveEcliFromUrl(url) {
    if (!url) {
        return null;
    }
...
function extractEcliFromText(text) {
    if (!text) {
        return null;
    }
...
function minutesBetween(startIso, end) {
    if (!startIso) {
        return null;
    }
...
function bucketResolution(minutes) {
...
function parseEvidenceNotes(value) {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
...
async function refreshFriaEvidence(orgId, actorId) {
    ...
    const releases = Array.from(new Set(artifacts
...
function resolveDateRange(startParam, endParam) {
    const now = new Date();
...
app.get('/launch/digests', async (request, reply) => {
    const userHeader = request.headers['x-user-id'];
    ...
    const digests = await withRequestSpan(request, {
    ...
    try {
        ...
        const [overviewResult, toolResult, provenanceResult, identifierResult, jurisdictionResult, manifestResult] = await Promise.all([
            ...
            supabase
                ...
                .select('jurisdiction_code, residency_zone, total_sources, sources_consolidated, sources_with_binding, sources_with_language_note, sources_with_eli, sources_with_ecli, sources_with_akoma, binding_breakdown, source_type_breakdown, language_note_breakdown')
                .eq('org_id', orgId)
                .order('jurisdiction_code', { ascending: true }),
            supabase
        ...
        const manifest = manifestRow
            ? (() => {
                const fileCount = manifestRow.file_count ?? 0;
                const validCount = manifestRow.valid_count ?? 0;
                const warningCount = manifestRow.warning_count ?? 0;
                const errorCount = manifestRow.error_count ?? 0;
                const validated = Boolean(manifestRow.validated);
                let status = 'ok';
                if (errorCount > 0)
                    status = 'errors';
                    ...
                    status = 'ok';
                return {
                    manifestName: manifestRow.manifest_name ?? null,
                    manifestUrl: manifestRow.manifest_url ?? null,
                    fileCount,
                    validCount,
                    warningCount,
                    errorCount,
...
app.get('/governance/publications', async (request, reply) => {
...
app.get('/metrics/retrieval', {
    ...
    try {
        ...
        return buildRetrievalMetricsResponse((summaryResult.data ?? null), (originResult.data ?? []), (hostResult.data ?? []));
    }
    catch (error) {
        if (error instanceof Error && 'statusCode' in error && typeof error.statusCode === 'number') {
            return reply.code(error.statusCode).send({ error: error.message });
    ...
    try {
        ...
        const [summaryResult, jurisdictionResult] = await Promise.all([
            ...
            supabase
                ...
                .select('jurisdiction, evaluation_count, pass_rate, citation_precision_median, temporal_validity_median, avg_binding_warnings, maghreb_banner_coverage')
                .eq('org_id', orgId),
        ]);
        if (summaryResult.error) {
        ...
        return buildEvaluationMetricsResponse((summaryResult.data ?? null), (jurisdictionResult.data ?? []));
    }
    catch (error) {
        if (error instanceof Error && 'statusCode' in error && typeof error.statusCode === 'number') {
            return reply.code(error.statusCode).send({ error: error.message });
...
app.get('/metrics/cepej', {
    ...
    try {
        ...
        const summary = summariseCepej((data ?? []));
...
app.get('/metrics/cepej/export', async (request, reply) => {
...
app.post('/reports/transparency', async (request, reply) => {
    ...
    try {
        ...
        if (caseIds.length > 0) {
            ...
            evaluationRows = (evalResult.data ?? []);
        }
        if (runResult.error) {
        ...
        const runSummary = summariseRuns((runResult.data ?? []));
        const hitlSummary = summariseHitl((hitlResult.data ?? []));
        const ingestionSummary = summariseIngestion((ingestionResult.data ?? []));
        const evaluationSummary = summariseEvaluations(evaluationRows);
        const cepejSummary = summariseCepej((cepejResult.data ?? []));
...
app.get('/reports/transparency', async (request, reply) => {
    const { orgId } = request.query;
...
app.get('/reports/transparency/:reportId', async (request, reply) => {
    ...
    try {
        await authorizeRequestWithGuards('governance:transparency', data.org_id, userHeader, request);
        return { report: data };
    }
    catch (authError) {
...
app.get('/admin/org/:orgId/red-team', async (request, reply) => {
...
app.post('/admin/org/:orgId/red-team', async (request, reply) => {
    const { orgId } = request.params;
    const userHeader = request.headers['x-user-id'];
    if (!userHeader || typeof userHeader !== 'string') {
        return reply.code(400).send({ error: 'x-user-id header is required' });
...
app.patch('/admin/org/:orgId/red-team/:findingId', async (request, reply) => {
...
app.get('/admin/org/:orgId/go-no-go/fria', async (request, reply) => {
    const { orgId } = request.params;
    const userHeader = request.headers['x-user-id'];
    if (!userHeader || typeof userHeader !== 'string') {
        return reply.code(400).send({ error: 'x-user-id header is required' });
    ...
    try {
        ...
        const { data, error } = await supabase
            .from('fria_artifacts')
            .select('id, release_tag, title, evidence_url, storage_path, hash_sha256, validated, submitted_by, submitted_at, notes')
            .eq('org_id', orgId)
            .order('submitted_at', { ascending: false });
        if (error) {
...
app.post('/admin/org/:orgId/go-no-go/fria', async (request, reply) => {
    const { orgId } = request.params;
    const userHeader = request.headers['x-user-id'];
    if (!userHeader || typeof userHeader !== 'string') {
        return reply.code(400).send({ error: 'x-user-id header is required' });
...
app.patch('/admin/org/:orgId/go-no-go/fria/:artifactId', async (request, reply) => {
...
app.get('/admin/org/:orgId/go-no-go/evidence', async (request, reply) => {
    const { orgId } = request.params;
    const userHeader = request.headers['x-user-id'];
    if (!userHeader || typeof userHeader !== 'string') {
        return reply.code(400).send({ error: 'x-user-id header is required' });
...
app.post('/admin/org/:orgId/go-no-go/evidence', async (request, reply) => {
    const { orgId } = request.params;
    const userHeader = request.headers['x-user-id'];
    if (!userHeader || typeof userHeader !== 'string') {
        return reply.code(400).send({ error: 'x-user-id header is required' });
    ...
    const { section, criterion, status, evidenceUrl, notes } = parsed.data;
...
app.patch('/admin/org/:orgId/go-no-go/evidence/:evidenceId', async (request, reply) => {
...
app.get('/admin/org/:orgId/go-no-go/signoffs', async (request, reply) => {
    const { orgId } = request.params;
    const userHeader = request.headers['x-user-id'];
    if (!userHeader || typeof userHeader !== 'string') {
        return reply.code(400).send({ error: 'x-user-id header is required' });
...
app.post('/admin/org/:orgId/go-no-go/signoffs', async (request, reply) => {
    const { orgId } = request.params;
    const userHeader = request.headers['x-user-id'];
    if (!userHeader || typeof userHeader !== 'string') {
        return reply.code(400).send({ error: 'x-user-id header is required' });
...
app.get('/admin/org/:orgId/performance/snapshots', async (request, reply) => {
    const { orgId } = request.params;
    const userHeader = request.headers['x-user-id'];
    if (!userHeader || typeof userHeader !== 'string') {
        return reply.code(400).send({ error: 'x-user-id header is required' });
...
app.post('/admin/org/:orgId/performance/snapshots', async (request, reply) => {
    const { orgId } = request.params;
    const userHeader = request.headers['x-user-id'];
    if (!userHeader || typeof userHeader !== 'string') {
        return reply.code(400).send({ error: 'x-user-id header is required' });
...
app.delete('/admin/org/:orgId/sso/:connectionId', {
    schema: {
        params: {
            type: 'object',
...
app.get('/metrics/slo', {
    ...
    try {
        ...
        const query = supabase
            .from('slo_snapshots')
            .select('captured_at, api_uptime_percent, hitl_response_p95_seconds, retrieval_latency_p95_seconds, citation_precision_p95, notes')
            .eq('org_id', orgId)
...
app.post('/metrics/slo', async (request, reply) => {
    const bodySchema = z.object({
        orgId: z.string().uuid(),
    ...
    const { orgId, apiUptimePercent, hitlResponseP95Seconds, retrievalLatencyP95Seconds, citationPrecisionP95, notes } = parsed.data;
    const userHeader = request.headers['x-user-id'];
    if (!userHeader || typeof userHeader !== 'string') {
        return reply.code(400).send({ error: 'x-user-id header is required' });
    }
...
app.get('/reports/learning', {
    ...
    return { reports: mapLearningReports((data ?? [])) };
});
app.get('/reports/dispatches', async (request, reply) => {
...
app.get('/admin/org/:orgId/operations/overview', {
    schema: {
        params: { type: 'object', properties: { orgId: { type: 'string' } }, required: ['orgId'] },
        headers: { type: 'object', properties: { 'x-user-id': { type: 'string' } }, required: ['x-user-id'] },
    },
    ...
    try {
        ...
        const [sloResult, incidentResult, changeResult, evidenceResult, regulatorPublication] = await Promise.all([
            ...
            supabase
                ...
                .select('id, occurred_at, detected_at, resolved_at, severity, status, title, summary, impact, resolution, follow_up, evidence_url, recorded_at')
                .eq('org_id', orgId)
        ...
        if (sloError || incidentError || changeError || evidenceError || regulatorError) {
            request.log.error({ sloError, incidentError, changeError, evidenceError, regulatorError }, 'operations overview query failed');
            return reply.code(500).send({ error: 'operations_overview_failed' });
        }
        const sloRows = (sloResult.data ?? []);
        const sloSummary = summariseSlo(sloRows);
        const sloSnapshots = sloRows.slice(0, 5);
        const incidentRows = (incidentResult.data ?? []);
...
app.post('/reports/dispatches', async (request, reply) => {
    const bodySchema = z.object({
        orgId: z.string().uuid(),
...
app.get('/admin/org/:orgId/scim-tokens', {
    schema: {
        params: { type: 'object', properties: { orgId: { type: 'string' } }, required: ['orgId'] },
        headers: { type: 'object', properties: { 'x-user-id': { type: 'string' } }, required: ['x-user-id'] },
    },
...
app.post('/admin/org/:orgId/scim-tokens', {
    schema: {
        params: { type: 'object', properties: { orgId: { type: 'string' } }, required: ['orgId'] },
        headers: { type: 'object', properties: { 'x-user-id': { type: 'string' } }, required: ['x-user-id'] },
        body: {
...
app.delete('/admin/org/:orgId/scim-tokens/:tokenId', {
    schema: {
        params: {
            type: 'object',
...
app.get('/admin/org/:orgId/audit-events', async (request, reply) => {
    const { orgId } = request.params;
...
app.get('/admin/org/:orgId/ip-allowlist', async (request, reply) => {
    const { orgId } = request.params;
    const userHeader = request.headers['x-user-id'];
    if (!userHeader || typeof userHeader !== 'string') {
        return reply.code(400).send({ error: 'x-user-id header is required' });
...
app.post('/admin/org/:orgId/ip-allowlist', async (request, reply) => {
    const { orgId } = request.params;
...
app.patch('/admin/org/:orgId/ip-allowlist/:entryId', async (request, reply) => {
...
app.delete('/admin/org/:orgId/ip-allowlist/:entryId', async (request, reply) => {
    const { orgId, entryId } = request.params;
    const userHeader = request.headers['x-user-id'];
    if (!userHeader || typeof userHeader !== 'string') {
        return reply.code(400).send({ error: 'x-user-id header is required' });
...
function scimError(reply, status, detail) {
    return reply
        .code(status)
...
app.get('/citations', async (request, reply) => {
    const { orgId } = request.query;
    if (!orgId) {
        return reply.code(400).send({ error: 'orgId is required' });
    }
...
app.get('/case-scores', async (request, reply) => {
...
app.get('/case-treatments', async (request, reply) => {
    const { orgId, sourceId } = request.query;
    if (!orgId) {
        return reply.code(400).send({ error: 'orgId is required' });
    }
...
app.get('/case-statute-links', async (request, reply) => {
    const { orgId, sourceId } = request.query;
    if (!orgId) {
        return reply.code(400).send({ error: 'orgId is required' });
    }
...
app.get('/drafting/templates', async (request, reply) => {
...
app.get('/hitl/metrics', async (request, reply) => {
    const { orgId } = request.query;
    if (!orgId) {
        return reply.code(400).send({ error: 'orgId is required' });
    }
        ...
        if (kind === 'fairness' && payload) {
            ...
            const benchmarks = Array.isArray(payload.benchmarks)
                ? payload.benchmarks
                    ...
                    .filter((entry) => entry !== null)
                : [];
            const flagged = payload.flagged && typeof payload.flagged === 'object'
                ? {
                    jurisdictions: Array.isArray(payload.flagged.jurisdictions)
                        ? payload.flagged.jurisdictions
                            .filter((code) => typeof code === 'string')
                        : [],
                    benchmarks: Array.isArray(payload.flagged.benchmarks)
                        ? payload.flagged.benchmarks
            ...
            fairnessReports.push({
                ...
                windowStart,
                windowEnd,
                overall: payload.overall && typeof payload.overall === 'object'
                    ? normaliseFairnessOverall(payload.overall)
                    : null,
                jurisdictions,
                benchmarks,
                flagged,
    ...
    const queue = queuePayload
        ? {
            ...
            oldestCreatedAt: typeof queuePayload.oldestCreatedAt === 'string' ? queuePayload.oldestCreatedAt : null,
            capturedAt: typeof queuePayload.capturedAt === 'string'
    ...
    const fairness = fairnessPayload
        ? {
            ...
            overall: fairnessPayload.overall && typeof fairnessPayload.overall === 'object'
                ? normaliseFairnessOverall(fairnessPayload.overall)
                : null,
            capturedAt: typeof fairnessPayload.capturedAt === 'string'
            ...
            benchmarks: Array.isArray(fairnessPayload.benchmarks)
                ? fairnessPayload.benchmarks
                    ...
                    .filter((entry) => entry !== null)
                : [],
            flagged: fairnessPayload.flagged && typeof fairnessPayload.flagged === 'object'
                ? {
                    jurisdictions: Array.isArray(fairnessPayload.flagged.jurisdictions)
                        ? fairnessPayload.flagged.jurisdictions
                            .filter((code) => typeof code === 'string')
                        : [],
                    benchmarks: Array.isArray(fairnessPayload.flagged.benchmarks)
                        ? fairnessPayload.flagged.benchmarks
                            .filter((name) => typeof name === 'string')
                        : [],
                }
                : { jurisdictions: [], benchmarks: [] },
...
app.get('/hitl/:id', async (request, reply) => {
    ...
    const runId = hitlRecord.data.run_id;
...
app.post('/hitl/:id', async (request, reply) => {
    ...
    const previousPayload = runLookup.data && typeof runLookup.data === 'object' && 'irac' in runLookup.data
        ? runLookup.data.irac
        : null;
    const runOrgIdCandidate = (runLookup.data && typeof runLookup.data === 'object' && runLookup.data.org_id
        ? runLookup.data.org_id
        : null) ?? (typeof existing.org_id === 'string' ? existing.org_id : null);
    const sanitizedRevisedPayload = revisedPayload && typeof revisedPayload === 'object' ? revisedPayload : null;
    const now = new Date();
    const nowIso = now.toISOString();
    const rawMinutes = minutesBetween(existing.created_at, now);
...
app.get('/matters', async (request, reply) => {
    const { orgId } = request.query;
    if (!orgId) {
        return reply.code(400).send({ error: 'orgId is required' });
    }
    ...
    const matters = (data ?? []).map((row) => ({
        ...
        conclusion: typeof row.irac === 'object' ? row.irac.conclusion ?? null : null,
    }));
    return { matters };
});
...
app.post('/upload', async (request, reply) => {
    ...
    const userId = body.userId ?? request.headers['x-user-id'];
...
app.get('/corpus', async (request, reply) => {
    ...
    const [domains, snapshots, uploads, ingestions, summaries] = await Promise.all([
        ...
        supabase
            .from('documents')
            .select('id, name, storage_path, residency_zone, vector_store_status, vector_store_synced_at, created_at, bytes, mime_type, summary_status, summary_generated_at, summary_error, chunk_count, bucket_id, source_id')
            .eq('org_id', orgId)
    ...
    const summaryMap = new Map((summaries.data ?? []).map((row) => [row.document_id, row]));
    const allowedResidencyZones = collectAllowedResidencyZones(access);
    const activeResidencyZone = access.policies.residencyZone ?? allowedResidencyZones[0] ?? null;
    return {
...
app.post('/corpus/:documentId/resummarize', async (request, reply) => {
    const bodySchema = z.object({
        orgId: z.string().uuid(),
    ...
    const { data: source } = await supabase
        .from('sources')
        .select('title, publisher, jurisdiction_code, source_url, adopted_date, effective_date, binding_lang, language_note, consolidated, eli, ecli, akoma_ntoso')
    ...
    let summaryError = result.error ?? null;
    ...
    if (finalStatus === 'ready') {
        ...
        if (finalStatus === 'ready' && document.source_id) {
            ...
            const articleCandidates = result.chunks.filter((chunk) => typeof chunk.marker === 'string' && chunk.marker.length > 0);
            ...
            const existingAkoma = source && typeof source.akoma_ntoso === 'object' && source.akoma_ntoso
                ? source.akoma_ntoso
                : null;
            const consolidatedFlag = typeof source?.consolidated === 'boolean'
                ? source.consolidated
                : typeof existingAkoma?.meta?.publication?.consolidated === 'boolean'
                    ? existingAkoma.meta?.publication?.consolidated
                    : null;
...
app.post('/security/devices/revoke', { schema: { headers: { type: 'object', properties: { 'x-user-id': { type: 'string' } }, required: ['x-user-id'] } }, preHandler: [securityRateLimit] }, async (request, reply) => {
    const bodySchema = z.object({
        orgId: z.string().uuid(),
...
app.get('/corpus/diff', async (request, reply) => {
...
app.patch('/corpus/allowlist/:host', async (request, reply) => {
