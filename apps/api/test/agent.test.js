...
function createAsyncQuery(initialData = [], initialError = null) {
    ...
    builder.then = (resolve) => resolve(builder.__response);
    return builder;
}
const toolInvocationsSelectBuilder = {};
...
const templateQuery = {
    ...
    then: (resolve) => resolve({ data: templateRows, error: null }),
};
...
describe('runLegalAgent', () => {
    ...
    it('throws when a citation is not allowlisted', async () => {
        ...
        const { runLegalAgent } = await import('../src/agent.js');
        await expect(runLegalAgent({
            question: 'Analyse',
            orgId: '00000000-0000-0000-0000-000000000000',
            userId: '00000000-0000-0000-0000-000000000000',
        }, makeContext())).rejects.toThrow(/hors périmètre/);
    });
    ...
    it('disables web search when confidential mode is active', async () => {
        ...
        const agentInstance = runMock.mock.calls[0]?.[0];
    ...
    it('augments hybrid retrieval queries with learned synonyms', async () => {
        ...
        const embeddingCall = global.fetch.mock.calls.find((call) => (typeof call[0] === 'string' ? call[0] : call[0].toString()).includes('/embeddings'));
    ...
    it('blocks judge analytics queries for France and escalates to HITL', async () => {
        ...
        const { runLegalAgent } = await import('../src/agent.js');
        const result = await runLegalAgent({
            question: 'Peux-tu analyser les statistiques et classer les juges de la Cour d\'appel de Paris selon leurs décisions ?',
            orgId: '00000000-0000-0000-0000-000000000000',
            userId: '00000000-0000-0000-0000-000000000000',
        }, makeContext());
        expect(runMock).not.toHaveBeenCalled();
    ...
    it('requires a FRIA checkpoint for EU litigation scenarios', async () => {
        ...
        const { runLegalAgent } = await import('../src/agent.js');
        await runLegalAgent({
            question: "Prépare une requête introductive d'instance devant le tribunal judiciaire de Paris pour contester une sanction disciplinaire.",
            orgId: '00000000-0000-0000-0000-000000000000',
            userId: '00000000-0000-0000-0000-000000000000',
        }, makeContext());
        expect(hitlInsertMock).toHaveBeenCalled();
        const learningBatch = learningInsertMock.mock.calls.at(-1)?.[0] ?? [];
        const hasFriaTicket = learningBatch.some((job) => job.type === 'compliance_fria_ticket');
        expect(hasFriaTicket).toBe(true);
        const auditBatch = auditInsertMock.mock.calls.at(-1)?.[0] ?? [];
        const hasFriaAudit = auditBatch.some((event) => event.kind === 'compliance.eu_ai_act.fria_required');
