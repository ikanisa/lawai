...
async function fetchLatestConsent(
  ...
  return { version: data.version as string, type: (data.consent_type as string) ?? consentType };
}
...
export async function authorizeAction(
  ...
  const [latestConsent, latestCoe] = await Promise.all([
    ...
    councilRequirement?.version
      ? fetchLatestConsent(orgId, userId, 'coe_disclosure')
...
export function ensureOrgAccessCompliance(access: OrgAccessContext, request: RequestContext): void {
  ...
  if (
    consentRequirement &&
    (!access.consent.latest || access.consent.latest.version !== consentRequirement.version)
  ) {
