...
function mapComplianceRow(
  ...
  const friaReasonsRaw = row.fria_reasons;
  const friaReasons = Array.isArray(friaReasonsRaw)
    ? (friaReasonsRaw as unknown[]).filter((item): item is string => typeof item === 'string')
    : typeof friaReasonsRaw === 'string'
      ? [friaReasonsRaw]
      : [];

  const cepejViolationsRaw = row.cepej_violations;
  const cepejViolations = Array.isArray(cepejViolationsRaw)
    ? (cepejViolationsRaw as unknown[]).filter((item): item is string => typeof item === 'string')
    : typeof cepejViolationsRaw === 'string'
      ? [cepejViolationsRaw]
      : [];
  ...
  return {
    ...
    cepej: {
      passed: row.cepej_passed === null ? true : Boolean(row.cepej_passed),
      violations: cepejViolations,
    },
  } satisfies ComplianceAssessment;
}

export async function loadComplianceAssessmentForRun(
  orgId: string,
  runId: string,
): Promise<ComplianceAssessment | null> {
  const { data, error } = await supabase
    .from('compliance_assessments')
    .select('fria_required, fria_reasons, cepej_passed, cepej_violations')
    .eq('org_id', orgId)
    .eq('run_id', runId)
    .maybeSingle();

  if (error) {
    console.warn('compliance_assessment_lookup_failed', error.message);
    return null;
  }

  return mapComplianceRow(data ?? undefined);
}

export async function loadLatestComplianceAssessment(orgId: string): Promise<ComplianceAssessment | null> {
  const { data, error } = await supabase
    .from('compliance_assessments')
    .select('fria_required, fria_reasons, cepej_passed, cepej_violations')
    .eq('org_id', orgId)
  ...
  if (error) {
    console.warn('latest_compliance_assessment_lookup_failed', error.message);
    return null;
  }
...
export async function buildComplianceSummary(
  ...
  const consentLatest = accessContext?.consent.latest ?? null;
  const consentAccepted = consentRequirement
    ? Boolean(consentLatest && consentLatest.version === consentRequirement.version)
    : true;

  const councilRequirement = accessContext?.policies.councilOfEuropeRequirement ?? null;
  const councilAcknowledged = councilRequirement?.version
    ? accessContext?.councilOfEurope.acknowledgedVersion === councilRequirement.version
    : true;

  const needsFriaArtifacts =
    Boolean(assessment?.fria.required) || Boolean(councilRequirement?.version) || Boolean(consentRequirement);
  ...
  if (needsFriaArtifacts) {
    ...
    friaArtifacts = (friaRows ?? []).map((row) => ({
      id: (row?.id as string) ?? `fria:${Math.random().toString(36).slice(2)}`,
      title: (row?.title as string) ?? 'FRIA',
  ...
  return {
    friaRequired: Boolean(assessment?.fria.required),
    friaValidated,
    ...
    consent: {
      ...
      latestVersion: consentLatest?.version ?? null,
    },
    councilOfEurope: {
      requirement: councilRequirement ?? null,
      acknowledged: councilAcknowledged,
      acknowledgedVersion: accessContext?.councilOfEurope.acknowledgedVersion ?? null,
    },
  };
}
...
async function fetchTrustPanelForRun(
  ...
  try {
    ...
    const [caseSummaries, assessment] = await Promise.all([
      loadCaseQualitySummaries(orgId, payload.citations),
      loadComplianceAssessmentForRun(orgId, runId),
    ]);
    const forceHitl = caseSummaries.some((item) => item.hardBlock || item.score < 55);
    const complianceSummary = await buildComplianceSummary(orgId, assessment, accessContext);
...
async function persistRun(
      ...
      if (existing.data?.id) {
        const restoredTrust =
          ...
          buildTrustPanel(
            ...
            verification,
            await buildComplianceSummary(
              input.orgId,
              await loadComplianceAssessmentForRun(input.orgId, existing.data.id as string),
              accessContext,
            ),
          );
  ...
  const complianceSummary = await buildComplianceSummary(input.orgId, complianceAssessment, accessContext);
  const trustPanel = buildTrustPanel(payload, retrievalSnippets, caseQuality, verification, complianceSummary);
...
export async function runLegalAgent(
  ...
  if (existing) {
    ...
    const existingAssessment = await loadComplianceAssessmentForRun(input.orgId, existing.id);
