...
export function registerOrchestratorRoutes(app: FastifyInstance, ctx: AppContext): void {
  ...
  app.get<{ Params: { id: string }; Querystring: ListCommandsQuery }>('/agent/sessions/:id/commands', async (request, reply) => {
  ...
  app.post<{ Body: unknown }>('/agent/commands', async (request, reply) => {
