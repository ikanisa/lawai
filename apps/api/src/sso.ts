...
function sanitizeConnection(row: Record<string, any>) {
  return {
    id: row.id as string,
    ...
    acsUrl: row.acs_url as string | null,
    entityId: row.entity_id as string | null,
    clientId: row.client_id as string | null,
    defaultRole: row.default_role as string,
...
export async function resolveScimToken(rawToken: string) {
  const tokenHash = hashToken(rawToken);
  const { data, error } = await supabase
    .from('scim_tokens')
  ...
  const expiresAt = data.expires_at ? Date.parse(data.expires_at as string) : null;
