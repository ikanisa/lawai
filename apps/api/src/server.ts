...
import {
  ...
  loadLatestComplianceAssessment,
} from './agent.js';
...
app.get<{ Params: { orgId: string } }>('/org/:orgId/compliance/status', async (request, reply) => {
  ...
  try {
    const access = await authorizeRequestWithGuards('runs:execute', orgId, userHeader, request);
    const assessment = await loadLatestComplianceAssessment(orgId);
    const summary = await buildComplianceSummary(orgId, assessment, access);
    return { compliance: summary };
  } catch (error) {
    if (error instanceof Error && 'statusCode' in error && typeof error.statusCode === 'number') {
      return reply.code(error.statusCode).send({ error: error.message });
    }
    request.log.error({ err: error }, 'compliance status lookup failed');
    return reply.code(500).send({ error: 'compliance_status_failed' });
  }
});
...
app.post<{
  Params: { orgId: string };
  Body: {
    consent?: { type?: string | null; version?: string | null } | null;
    councilOfEurope?: { version?: string | null } | null;
  };
}>('/org/:orgId/compliance/acknowledgements', async (request, reply) => {
  const { orgId } = request.params;
  if (!orgId) {
    return reply.code(400).send({ error: 'orgId is required' });
  }
  ...
  const councilPayload = body.councilOfEurope;

  if (!consentPayload && !councilPayload) {
    return reply.code(400).send({ error: 'no_acknowledgements_supplied' });
  }

  if (consentPayload && (!consentPayload.type || !consentPayload.version)) {
    return reply.code(400).send({ error: 'invalid_consent_payload' });
  }

  if (councilPayload && !councilPayload.version) {
    return reply.code(400).send({ error: 'invalid_council_payload' });
  }

  try {
    const access = await authorizeRequestWithGuards('runs:execute', orgId, userHeader, request);

    const inserts: Array<{ org_id: string; user_id: string; consent_type: string; version: string }> = [];

    if (consentPayload?.type && consentPayload.version) {
      inserts.push({
        org_id: orgId,
        user_id: userHeader,
        consent_type: consentPayload.type,
        version: consentPayload.version,
      });
    }

    if (councilPayload?.version) {
      inserts.push({
        org_id: orgId,
        user_id: userHeader,
        consent_type: 'coe_disclosure',
        version: councilPayload.version,
      });
    }

    if (inserts.length > 0) {
      const { error } = await supabase.from('consent_events').insert(inserts);
      if (error) {
        request.log.error({ err: error }, 'consent acknowledgement insert failed');
        return reply.code(500).send({ error: 'compliance_ack_failed' });
      }
    }

    if (consentPayload?.type && consentPayload.version) {
      access.consent.latest = { type: consentPayload.type, version: consentPayload.version };
    }
    if (councilPayload?.version) {
      access.councilOfEurope.acknowledgedVersion = councilPayload.version;
    }

    await logAuditEvent({
      orgId,
      actorUserId: userHeader,
      kind: 'compliance.acknowledgement.recorded',
      object: `org:${orgId}`,
      metadata: {
        consent: consentPayload ?? null,
        councilOfEurope: councilPayload ?? null,
      },
    });
    ...
    request.log.error({ err: error }, 'compliance acknowledgement failed');
    return reply.code(500).send({ error: 'compliance_ack_failed' });
  }
});
