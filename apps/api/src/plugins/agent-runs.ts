...
export const agentRunsPlugin = fp<AgentRunsPluginOptions>(async (app, options) => {
  ...
  const runExecutionRateLimit = createRateLimitPreHandler({
    ...
    keyGenerator: (request) => {
      ...
      const userHeader =
        typeof request.headers['x-user-id'] === 'string' ? (request.headers['x-user-id'] as string) : undefined;
  ...
  app.post<{ Body: z.infer<typeof requestSchema> }>(
    '/runs',
    { preHandler: [runExecutionRateLimit] },
    async (request, reply) => {
      ...
      try {
        ...
        const result = await timeoutGuard.run(() =>
          runLegalAgent(
            {
              question,
        ...
        const responsePayload: IRACPayload = safePayload.success
          ? (safePayload.data as unknown as IRACPayload)
          : ((result.payload ?? {}) as IRACPayload);
