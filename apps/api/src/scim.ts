...
interface OrgMemberRow {
  ...
  updated_at?: string | null;
  profiles?: OrgMemberProfile | null;
}
...
function parseRoleFromPayload(payload: ScimUserPayload | null | undefined): string {
...
function extractEmail(payload: ScimUserPayload | null | undefined): string | null {
...
function extractName(payload: ScimUserPayload | null | undefined): {
...
async function upsertProfile(userId: string, payload: ScimUserPayload | null | undefined) {
...
function formatScimUser(row: OrgMemberRow) {
...
async function resolveBearer(authHeader: string): Promise<ScimContext> {
...
export async function createScimUser(authHeader: string, payload: ScimUserPayload) {
  const context = await resolveBearer(authHeader);
  ...
  const fallbackRow: OrgMemberRow = {
    user_id: userId,
    role,
    profiles: {
  ...
  return formatScimUser((data as OrgMemberRow | null | undefined) ?? fallbackRow);
}

async function ensureMembership(orgId: string, userId: string): Promise<OrgMemberRow> {
  const { data, error } = await supabase
    .from('org_members')
...
export async function patchScimUser(authHeader: string, userId: string, payload: ScimPatchRequest) {
  const context = await resolveBearer(authHeader);
  const membership = await ensureMembership(context.orgId, userId);

  let active: boolean | null = null;
  let newRole: string | null = null;
  let profileUpdates: Partial<ScimUserPayload> | null = null;
    ...
    if (op === 'replace') {
      if (path === 'active' || path === 'urn:ietf:params:scim:schemas:core:2.0:user:active') {
        ...
        if (typeof value === 'boolean') {
          active = value;
        } else if (typeof value === 'object' && value && 'active' in (value as Record<string, unknown>)) {
          const maybeActive = (value as Record<string, unknown>).active;
        ...
        if (Array.isArray(value)) {
          newRole = parseRoleFromPayload({ roles: value as ScimRoleEntry[] });
        } else if (typeof value === 'string') {
          newRole = parseRoleFromPayload({ role: value });
        }
      } else if (path.startsWith('emails') || path.includes('name')) {
        const update = (operation.value ?? operation.Value) as Partial<ScimUserPayload> | undefined;
        if (update && typeof update === 'object') {
          profileUpdates = { ...(profileUpdates ?? {}), ...update };
        }
      } else if (!path && typeof operation.value === 'object' && operation.value) {
        profileUpdates = { ...(profileUpdates ?? {}), ...(operation.value as Partial<ScimUserPayload>) };
      }
    }
  }

  if (profileUpdates) {
    await upsertProfile(userId, profileUpdates as ScimUserPayload);
  }
  ...
  const withUpdatedRole: OrgMemberRow = {
    ...refreshed,
