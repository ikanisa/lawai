...
import {
  ...
  type LaunchOfflineStatus,
  type LaunchReadinessAction,
  type LaunchReadinessSnapshot,
  type LaunchReadinessSeverity,
} from '@avocat-ai/shared';
import { getLaunchCollateral, listRegulatorDigests } from './launch.js';
import { listWebVitals, type WebVitalRecord } from './metrics.js';

const MAX_OUTBOX_ITEMS_PER_ORG = 100;
const outboxStore: LaunchOfflineOutboxItem[] = [];

export function enqueueOfflineOutboxItem(
  input: Omit<LaunchOfflineOutboxItem, 'id' | 'queuedAt' | 'status' | 'lastAttemptAt'> &
    Partial<Pick<LaunchOfflineOutboxItem, 'status' | 'queuedAt' | 'lastAttemptAt'>>,
): LaunchOfflineOutboxItem {
  const item: LaunchOfflineOutboxItem = {
    id: randomUUID(),
...
function pruneOutboxForOrg(orgId: string) {
  let seen = 0;
...
export function updateOfflineOutboxStatus(
  orgId: string,
  itemId: string,
  status: LaunchOfflineStatus,
  attemptAt?: string,
): LaunchOfflineOutboxItem | null {
...
export function listOfflineOutboxItems(orgId: string): LaunchOfflineOutboxItem[] {
  return outboxStore.filter((entry) => entry.orgId === orgId);
}

function summariseVitals(vitals: WebVitalRecord[]) {
  const counts = {
    total: vitals.length,
    good: 0,
    needsImprovement: 0,
...
function summariseOfflineOutbox(items: LaunchOfflineOutboxItem[]) {
...
function computeReadinessScore(
  vitals: ReturnType<typeof summariseVitals>,
  offline: ReturnType<typeof summariseOfflineOutbox>,
  digests: ReturnType<typeof summariseDigests>,
): number {
  let score = 100;
...
function buildActions(
  ...
  const actions: LaunchReadinessAction[] = [];

  if (vitals.poor > 0) {
    actions.push({
      id: 'core-web-vitals',
      severity: vitals.poor >= 3 ? ('critical' satisfies LaunchReadinessSeverity) : 'warning',
...
export function buildPhaseEReadiness(orgId: string): LaunchReadinessSnapshot {
