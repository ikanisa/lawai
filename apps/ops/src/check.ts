...
function auditSecrets(env: Record<string, string>): string[] {
  const failures: string[] = [];
  const weakValues = new Set(['test', 'service', 'changeme', '']);

  if (weakValues.has(env.OPENAI_API_KEY?.toLowerCase?.() ?? '')) {
    failures.push('OPENAI_API_KEY est manquant ou utilise une valeur de test.');
  }

  if ((env.SUPABASE_SERVICE_ROLE_KEY ?? '').length < 32 || weakValues.has(env.SUPABASE_SERVICE_ROLE_KEY ?? '')) {
    failures.push('SUPABASE_SERVICE_ROLE_KEY est absent ou trop court.');
  }

  if (!env.SUPABASE_URL || env.SUPABASE_URL.includes('example.supabase.co')) {
    failures.push('SUPABASE_URL n’est pas configurée vers votre projet.');
  }

  return failures;
}

async function checkExtensions(connectionString: string): Promise<string[]> {
  ...
  try {
    ...
    return REQUIRED_EXTENSIONS.filter((ext) => !present.has(ext));
  } finally {
    await client.end();
  }
}
...
async function main(): Promise<void> {
  ...
  try {
    const missingExtensions = await checkExtensions(env.SUPABASE_DB_URL);
