...
async function main() {
  try {
    ...
    if (warnings.length) {
      warnings.forEach((warning) => console.warn('[admin-pwa] workbox warning', warning));
    }

    await rm(intermediate, { force: true });

    console.log(`[admin-pwa] precached ${count} assets (${size} bytes)`);
  } catch (error) {
    console.error('[admin-pwa] failed to build service worker', error);
    process.exitCode = 1;
  }
}
