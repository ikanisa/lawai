...
export function WorkspaceView({ messages, locale }: WorkspaceViewProps) {
  ...
  const { data } = useWorkspaceOverview(locale);

  const matters = data?.matters ?? [];
  const complianceWatch = data?.complianceWatch ?? [];
  const hitlInbox = data?.hitlInbox ?? { items: [], pendingCount: 0 };
  const desk: WorkspaceDesk | undefined = data?.desk;
  const navigatorFlows = data?.navigator ?? [];

  const jurisdictionChips = useMemo(
    () => {
      const items = data?.jurisdictions ?? [];
    ...
    [data?.jurisdictions, locale, router],
  );
