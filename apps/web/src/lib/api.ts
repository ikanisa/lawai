...
export async function fetchComplianceStatus(
  ...
  const response = await fetch(`${API_BASE}/org/${orgId}/compliance/status`, {
    headers: { 'x-user-id': userId },
    cache: 'no-store',
  });
  if (!response.ok) {
    throw new Error('Unable to fetch compliance status');
  }
  return response.json();
}
...
export async function acknowledgeCompliance(
  ...
  const response = await fetch(`${API_BASE}/org/${orgId}/compliance/acknowledgements`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-user-id': userId },
    body: JSON.stringify(input ?? {}),
  });
  if (!response.ok) {
    throw new Error('Unable to record compliance acknowledgement');
  }
  return response.json();
}
