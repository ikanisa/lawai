...
const REDLINE_DIFF = [
  {
    id: 'r1',
    original: 'Le Fournisseur garantit la conformité des produits pour une durée de douze (12) mois à compter de la livraison.',
    revised:
    ...
    impact: 'Allonge la garantie – vérifier compatibilité avec Code de la consommation (FR).',
  },
  {
    id: 'r2',
    ...
    <div className="space-y-10">
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {REDLINE_DIFF.map((entry) => (
            <div key={entry.id} className="rounded-3xl border border-slate-800/60 bg-slate-950/70 p-5 shadow-lg">
              <p className="text-xs uppercase tracking-wide text-slate-400">Avant</p>
              <p className="mt-1 text-sm text-slate-300">{entry.original}</p>
              <p className="mt-4 text-xs uppercase tracking-wide text-slate-400">Après</p>
              <p className="mt-1 text-sm text-emerald-200">{entry.revised}</p>
              <div className="mt-4 flex items-center justify-between">
                <p className="text-xs text-slate-400">{entry.impact}</p>
