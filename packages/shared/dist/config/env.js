...
const runtimeNodeEnv = NODE_ENV_OPTIONS.includes(process.env.NODE_ENV ?? '')
  ? process.env.NODE_ENV
  : 'development';

export const nodeEnvironmentSchema = z
  .enum(NODE_ENV_OPTIONS)
  .default(runtimeNodeEnv ?? 'development');
...
export function loadServerEnv(schema, options = {}) {
  const { dotenv = process.env.NODE_ENV !== 'production', source = process.env } = options;
