...
export function summariseTransparencyReport(
  ...
  const runs = formatCount(operations.totalRuns ?? (operations as Record<string, number | null>).total_runs);
  const hitlTotalCandidate =
    operations.hitlTriggered ??
    hitl.total ??
    (operations as Record<string, number | null>).hitl_total ??
    0;
  const hitlCount = formatCount(hitlTotalCandidate);
  const hitlMedian = formatDuration(hitl?.medianResponseMinutes ?? null, locale);
  const complianceRate =
    compliance && Object.prototype.hasOwnProperty.call(compliance, 'cepejPassRate')
      ? formatPercent(
          (compliance as { cepejPassRate?: number | null }).cepejPassRate ?? null,
          locale,
        )
      : 'n/a';
  const evalRate =
    evaluations && Object.prototype.hasOwnProperty.call(evaluations, 'passRate')
      ? formatPercent(
          (evaluations as { passRate?: number | null }).passRate ?? null,
          locale,
        )
      : 'n/a';
  const ingestionSummary = `${formatCount((ingestion as { succeeded?: number | null }).succeeded ?? 0)}/${formatCount(
    (ingestion as { total?: number | null }).total ?? 0,
  )}`;
...
export function formatTransparencyDigest(
  ...
  options: TransparencyDigestOptions = {},
  ...
  for (const report of reports) {
    lines.push(
      summariseTransparencyReport(report, {
        baseUrl,
        locale,
      }),
    );
