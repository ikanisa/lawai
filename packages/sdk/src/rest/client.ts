...
let apiBase = '';
let fallbackOrgId = DEMO_ORG_ID;
let fallbackUserId = DEMO_USER_ID;
let customFetch: typeof fetch | null = null;

function ensureConfigured() {
  if (!apiBase) {
    throw new Error('REST client not configured. Call createRestClient or configureRestClient first.');
  }
}

const restFetch = (...args: Parameters<typeof fetch>) => {
  const impl = customFetch ?? globalThis.fetch.bind(globalThis);
  return impl(...args);
};
...
export interface CreatedSloSnapshot extends SloSnapshot {
  org_id: string;
...
export type TransparencyReport = Record<string, unknown>;
...
export async function submitResearchQuestion(input: {
  ...
  const response = await restFetch(`${apiBaseUrl()}/runs`, {
    method: 'POST',
    headers: {
...
export async function requestHitlReview(
  ...
  const response = await restFetch(`${apiBaseUrl()}/runs/${encodeURIComponent(runId)}/hitl`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({
      orgId: input.orgId ?? fallbackOrgId,
...
export async function sendTelemetryEvent(
  ...
  try {
    await restFetch(`${apiBaseUrl()}/telemetry`, {
      method: 'POST',
...
export async function fetchCitations(orgId: string) {
  const response = await restFetch(`${apiBaseUrl()}/citations?orgId=${encodeURIComponent(orgId)}`);
  if (!response.ok) {
...
export async function fetchHitlQueue(orgId: string) {
  const response = await restFetch(`${apiBaseUrl()}/hitl?orgId=${encodeURIComponent(orgId)}`);
  if (!response.ok) {
...
export async function fetchHitlMetrics(orgId: string): Promise<HitlMetricsResponse> {
  const response = await restFetch(`${apiBaseUrl()}/hitl/metrics?orgId=${encodeURIComponent(orgId)}`, {
...
export async function fetchHitlDetail(orgId: string, id: string): Promise<HitlDetailResponse> {
  const response = await restFetch(`${apiBaseUrl()}/hitl/${encodeURIComponent(id)}?orgId=${encodeURIComponent(orgId)}`, {
    headers: { 'x-user-id': fallbackUserId },
  });
  if (!response.ok) {
...
export async function submitHitlAction(id: string, action: 'approve' | 'request_changes' | 'reject', comment?: string) {
  const response = await restFetch(`${apiBaseUrl()}/hitl/${id}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function fetchMatters(orgId: string) {
  const response = await restFetch(`${apiBaseUrl()}/matters?orgId=${encodeURIComponent(orgId)}`);
  if (!response.ok) {
...
export async function fetchMatterDetail(orgId: string, id: string) {
  const response = await restFetch(
    `${apiBaseUrl()}/matters/${encodeURIComponent(id)}?orgId=${encodeURIComponent(orgId)}`,
  );
  if (!response.ok) {
...
export async function fetchHitlAuditTrail(
  ...
  const url = new URL(`${apiBaseUrl()}/admin/org/${orgId}/audit-events`);
  if (options.limit) {
...
export async function fetchCorpus(orgId: string) {
  const response = await restFetch(`${apiBaseUrl()}/corpus?orgId=${encodeURIComponent(orgId)}`, {
    headers: { 'x-user-id': fallbackUserId },
  });
  if (!response.ok) {
...
export async function resummarizeDocument(
  ...
  const response = await restFetch(`${apiBaseUrl()}/corpus/${encodeURIComponent(documentId)}/resummarize`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
export async function fetchGovernanceMetrics(orgId: string): Promise<GovernanceMetricsResponse> {
  const response = await restFetch(`${apiBaseUrl()}/metrics/governance?orgId=${encodeURIComponent(orgId)}`, {
    headers: {
      'x-user-id': fallbackUserId,
    },
  });
...
export async function fetchRetrievalMetrics(orgId: string): Promise<RetrievalMetricsResponse> {
  const response = await restFetch(`${apiBaseUrl()}/metrics/retrieval?orgId=${encodeURIComponent(orgId)}`, {
    headers: {
      'x-user-id': fallbackUserId,
    },
  });
...
export async function fetchEvaluationMetrics(orgId: string): Promise<EvaluationMetricsResponse> {
  const response = await restFetch(`${apiBaseUrl()}/metrics/evaluations?orgId=${encodeURIComponent(orgId)}`, {
    headers: {
      'x-user-id': fallbackUserId,
    },
  });
...
export async function fetchSloMetrics(orgId: string, limit = 6): Promise<SloMetricsResponse> {
  ...
  const response = await restFetch(`${apiBaseUrl()}/metrics/slo?${params.toString()}`, {
    headers: {
      'x-user-id': fallbackUserId,
    },
  });
...
export async function exportSloSnapshots(
  orgId: string,
  options: { format?: 'json' | 'csv'; userId?: string } = {},
): Promise<SloMetricsResponse | string> {
  const params = new URLSearchParams({ orgId });
  if (options.format) {
    params.set('format', options.format);
  }
  const response = await restFetch(`${apiBaseUrl()}/metrics/slo/export?${params.toString()}`, {
    headers: {
      'x-user-id': options.userId ?? fallbackUserId,
    },
  });

  if (!response.ok) {
    throw new Error('Unable to export SLO snapshots');
  }
...
export async function createSloSnapshot(input: {
  ...
  citationPrecisionP95?: number | null;
  notes?: string | null;
}): Promise<CreatedSloSnapshot> {
  const response = await restFetch(`${apiBaseUrl()}/metrics/slo`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-user-id': input.userId ?? fallbackUserId,
    },
    body: JSON.stringify({
      orgId: input.orgId,
      ...
      citationPrecisionP95: input.citationPrecisionP95 ?? null,
  ...
  if (!response.ok) {
    ...
    throw new Error(`Unable to create SLO snapshot: ${response.status} ${text}`);
  }
...
export async function fetchSnapshotDiff(orgId: string, snapshotId: string, compareTo: string) {
  ...
  const response = await restFetch(`${apiBaseUrl()}/corpus/diff?${params.toString()}`);
  if (!response.ok) {
...
export async function toggleAllowlistDomain(host: string, active: boolean, jurisdiction?: string) {
  const response = await restFetch(`${apiBaseUrl()}/corpus/allowlist/${encodeURIComponent(host)}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function fetchSsoConnections(orgId: string): Promise<SsoConnectionResponse> {
  const response = await restFetch(`${apiBaseUrl()}/admin/org/${orgId}/sso`, {
    headers: { 'x-user-id': fallbackUserId },
  });
  if (!response.ok) {
...
export async function saveSsoConnection(
  ...
  const response = await restFetch(`${apiBaseUrl()}/admin/org/${orgId}/sso`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-user-id': fallbackUserId },
...
export async function removeSsoConnection(orgId: string, connectionId: string) {
  const response = await restFetch(`${apiBaseUrl()}/admin/org/${orgId}/sso/${connectionId}`, {
    method: 'DELETE',
    headers: { 'x-user-id': fallbackUserId },
  });
  if (!response.ok) {
...
export async function fetchScimTokens(orgId: string) {
  const response = await restFetch(`${apiBaseUrl()}/admin/org/${orgId}/scim-tokens`, {
    headers: { 'x-user-id': fallbackUserId },
  });
  if (!response.ok) {
...
export async function deleteScimAccessToken(orgId: string, tokenId: string) {
  const response = await restFetch(`${apiBaseUrl()}/admin/org/${orgId}/scim-tokens/${tokenId}`, {
    method: 'DELETE',
    headers: { 'x-user-id': fallbackUserId },
  });
  if (!response.ok) {
...
export async function fetchComplianceStatus(
  ...
  const response = await restFetch(`${apiBaseUrl()}/compliance/status?${params.toString()}`, {
    headers: {
      'x-user-id': options?.userId ?? fallbackUserId,
      'x-org-id': orgId,
    },
...
export async function acknowledgeCompliance(
  ...
  const response = await restFetch(`${apiBaseUrl()}/compliance/acknowledgements`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-user-id': input.userId ?? fallbackUserId,
...
export async function fetchAuditEvents(orgId: string, limit = 50) {
  const response = await restFetch(
    `${apiBaseUrl()}/admin/org/${orgId}/audit-events?limit=${encodeURIComponent(String(limit))}`,
    { headers: { 'x-user-id': fallbackUserId } },
  );
  if (!response.ok) {
    throw new Error('Unable to fetch audit events');
...
export async function fetchDeviceSessions(
  ...
  const response = await restFetch(`${apiBaseUrl()}/security/devices?${params.toString()}`, {
    headers: { 'x-user-id': fallbackUserId },
  });
  if (!response.ok) {
...
export async function revokeDeviceSession(orgId: string, sessionId: string, reason?: string) {
  const response = await restFetch(`${apiBaseUrl()}/security/devices/revoke`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-user-id': fallbackUserId,
...
export async function fetchIpAllowlist(orgId: string) {
  const response = await restFetch(`${apiBaseUrl()}/admin/org/${orgId}/ip-allowlist`, {
    headers: { 'x-user-id': fallbackUserId },
  });
  if (!response.ok) {
...
export async function upsertIpAllowlistEntry(
  ...
  const url = input.id
    ? `${apiBaseUrl()}/admin/org/${orgId}/ip-allowlist/${input.id}`
    : `${apiBaseUrl()}/admin/org/${orgId}/ip-allowlist`;
...
export async function deleteIpAllowlistEntry(orgId: string, entryId: string) {
  const response = await restFetch(`${apiBaseUrl()}/admin/org/${orgId}/ip-allowlist/${entryId}`, {
    method: 'DELETE',
    headers: { 'x-user-id': fallbackUserId },
  });
  if (!response.ok) {
...
export async function fetchDraftingTemplates(orgId: string, params?: { jurisdiction?: string; matterType?: string }) {
  ...
  const response = await restFetch(`${apiBaseUrl()}/drafting/templates?${search.toString()}`);
  if (!response.ok) {
...
export async function fetchWorkspaceOverview(orgId: string): Promise<WorkspaceOverviewResponse> {
  const response = await restFetch(`${apiBaseUrl()}/workspace?orgId=${encodeURIComponent(orgId)}`);
  if (!response.ok) {
...
export async function getOperationsOverview(orgId: string): Promise<OperationsOverviewResponse> {
  const response = await restFetch(`${apiBaseUrl()}/admin/org/${orgId}/operations/overview`, {
    headers: { 'x-user-id': fallbackUserId },
  });
  if (!response.ok) {
...
export async function getGovernancePublications(params?: {
  ...
  const url = query ? `${apiBaseUrl()}/governance/publications?${query}` : `${apiBaseUrl()}/governance/publications`;
  const response = await restFetch(url, { headers });
  if (!response.ok) {
...
export async function fetchLaunchDigests(
  orgId: string,
  options: { userId?: string; limit?: number } = {},
): Promise<LaunchDigestEntry[]> {
  const params = new URLSearchParams({ orgId, limit: String(options.limit ?? 25) });
  const response = await restFetch(`${apiBaseUrl()}/launch/digests?${params.toString()}`, {
    headers: {
      'x-user-id': options.userId ?? fallbackUserId,
      'x-org-id': orgId,
    },
...
export async function fetchDispatchReports(
  orgId: string,
  options: { userId?: string; periodStart?: string; periodEnd?: string } = {},
  ...
  const response = await restFetch(`${apiBaseUrl()}/reports/dispatches?${params.toString()}`, {
    headers: { 'x-user-id': options.userId ?? fallbackUserId },
  });

  if (!response.ok) {
    const body = await response.text();
    throw new Error(`Unable to fetch dispatch reports: ${response.status} ${body}`);
  }
...
export async function createTransparencyReport(input: {
  orgId: string;
  userId?: string;
  periodStart?: string;
  periodEnd?: string;
  ...
  const response = await restFetch(`${apiBaseUrl()}/reports/transparency`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-user-id': input.userId ?? fallbackUserId,
...
export function configureRestClient(options: RestClientOptions) {
  apiBase = options.baseUrl.replace(/\/+$/, '');
  customFetch = options.fetchImpl ?? null;
  fallbackOrgId = options.defaultOrgId ?? DEMO_ORG_ID;
  fallbackUserId = options.defaultUserId ?? DEMO_USER_ID;
  return restClient;
}

export function createRestClient(options: RestClientOptions) {
  return configureRestClient(options);
}
