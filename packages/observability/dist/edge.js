...
import { context, trace, SpanStatusCode, diag, DiagConsoleLogger, } from '@opentelemetry/api';
...
let telemetryRuntime = null;
let servePatched = false;
const deno = globalThis.Deno;
const getDenoEnv = (key) => deno?.env?.get?.(key);
...
function ensurePatchedServe(tracerName) {
    if (servePatched)
        return;
...
export async function initEdgeTelemetry(config) {
    if (telemetryRuntime) {
...
export async function withEdgeSpan(name, attributes, handler) {
