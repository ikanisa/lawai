...
DROP VIEW if EXISTS public.org_evaluation_metrics;
  ...
  LEFT JOIN (
    SELECT
      ...
      CASE
        WHEN count(*) FILTER (
          WHERE
            upper(coalesce(r.jurisdiction, '')) = 'RW'
        ) = 0 THEN NULL
        ELSE sum(
          CASE
            WHEN upper(coalesce(r.jurisdiction, '')) = 'RW'
            AND coalesce(r.rwanda_notice, FALSE) THEN 1
...
DROP VIEW if EXISTS public.org_evaluation_jurisdiction_metrics;
...
SELECT
  ...
  END AS maghreb_banner_coverage,
  CASE
    WHEN upper(coalesce(r.jurisdiction, '')) = 'RW' THEN sum(
      CASE
        WHEN coalesce(r.rwanda_notice, FALSE) THEN 1
        ELSE 0
      END
    )::numeric / nullif(count(r.id), 0)
    ELSE NULL
